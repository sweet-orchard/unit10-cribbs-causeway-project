...
const WHEEL_PAN_MULTIPLIER = 1.35;
const WHEEL_PAN_FINE_MULTIPLIER = 0.45;
const WHEEL_PAN_FAST_MULTIPLIER = 2.2;
...
window.addEventListener("DOMContentLoaded", initMobileDetailSheet);
...
function updateTransform() {
  ...
  window.dispatchEvent(new Event("map:transform"));
}
...
function zoomTo(nextZoom, clientX, clientY) {
  ...
  // Keep the map point under the anchor fixed while zooming.
  const mapX = (anchorX - state.panX) / state.zoom;
  const mapY = (anchorY - state.panY) / state.zoom;

  state.zoom = targetZoom;
  state.panX = anchorX - mapX * state.zoom;
  state.panY = anchorY - mapY * state.zoom;
  updateTransform();
}

function getVisibleMapArea() {
  const rect = mapViewport.getBoundingClientRect();
  const safe = { left: 0, top: 0, right: rect.width, bottom: rect.height };
  const overlays = [
    document.getElementById("sidebarPanel"),
    document.getElementById("detailView"),
    document.getElementById("mobileBottomMenuWrap"),
    document.getElementById("desktopBottomMenuWrap"),
  ].filter(Boolean);
  ...
  overlays.forEach((el) => {
    ...
    const left = Math.max(r.left, rect.left) - rect.left;
    const right = Math.min(r.right, rect.right) - rect.left;
    const top = Math.max(r.top, rect.top) - rect.top;
    const bottom = Math.min(r.bottom, rect.bottom) - rect.top;
    const w = right - left;
    const h = bottom - top;
    if (w <= 0 || h <= 0) return;

    if (top <= edgeSlack && w > rect.width * 0.4) {
      safe.top = Math.max(safe.top, bottom + margin);
    }
    if (bottom >= rect.height - edgeSlack && w > rect.width * 0.4) {
      safe.bottom = Math.min(safe.bottom, top - margin);
    }
    if (left <= edgeSlack && h > rect.height * 0.4) {
      safe.left = Math.max(safe.left, right + margin);
    }
    if (right >= rect.width - edgeSlack && h > rect.height * 0.4) {
      safe.right = Math.min(safe.right, left - margin);
    }
  });

  if (safe.right - safe.left < 40 || safe.bottom - safe.top < 40) {
    return {
      left: 0,
      top: 0,
      right: rect.width,
...
function ensureUnitVisible(unit) {
  if (!unit || !unit.getBBox) return;
  const bbox = unit.getBBox();
  const ctm = unit.getScreenCTM && unit.getScreenCTM();
  if (!ctm) return;

  const cx = bbox.x + bbox.width / 2;
  const cy = bbox.y + bbox.height / 2;
  const rect = mapViewport.getBoundingClientRect();
  const currentX = ctm.a * cx + ctm.c * cy + ctm.e - rect.left;
  const currentY = ctm.b * cx + ctm.d * cy + ctm.f - rect.top;
  const visible = getVisibleMapArea();

  const inside =
    currentX >= visible.left &&
    currentX <= visible.right &&
    currentY >= visible.top &&
    currentY <= visible.bottom;
  if (inside) return;

  state.panX += visible.cx - currentX;
  state.panY += visible.cy - currentY;
  updateTransform();
}
...
function setMobileDetailSheetOffset(offsetPx = 0) {
  const detailView = document.getElementById("detailView");
  if (!detailView) return;
  const safeOffset = Number.isFinite(offsetPx) ? Math.max(0, offsetPx) : 0;
  detailView.style.setProperty("--detail-drag-y", `${safeOffset}px`);
}

function initMobileDetailSheet() {
  const detailView = document.getElementById("detailView");
  const dragHandle = document.getElementById("detailDragHandle");
  const slider = document.getElementById("sidebarSlider");
  if (!detailView || !dragHandle || !slider) return;
  ...
  function maxOffset() {
    const panelHeight = detailView.getBoundingClientRect().height || 0;
    const peekVisiblePx = 72;
  ...
  function readOffset() {
    const raw = detailView.style.getPropertyValue("--detail-drag-y");
  ...
  function applyOffset(value) {
    ...
    setMobileDetailSheetOffset(currentOffset);
  }

  function beginDrag(clientY) {
    if (!isMobile() || !slider.classList.contains("show-detail")) return;
    dragging = true;
    startY = clientY;
    startOffset = readOffset();
    currentOffset = startOffset;
    detailView.classList.add("sheet-dragging");
  }
  ...
  function endDrag() {
    if (!dragging) return;
    dragging = false;
    detailView.classList.remove("sheet-dragging");

    const vh = window.innerHeight || document.documentElement.clientHeight || 0;
    const midOffset = Math.min(maxOffset(), Math.max(96, vh * 0.28));
    const lowOffset = maxOffset();

    if (currentOffset < midOffset * 0.45) {
      applyOffset(0);
      return;
    }
    if (currentOffset < (midOffset + lowOffset) / 2) {
      applyOffset(midOffset);
      return;
    }
    applyOffset(lowOffset);
  }
...
mapViewport.addEventListener(
  ...
  (e) => {
    ...
    let panScale = WHEEL_PAN_MULTIPLIER;
    if (e.altKey) panScale = WHEEL_PAN_FINE_MULTIPLIER;
    if (e.shiftKey) panScale = WHEEL_PAN_FAST_MULTIPLIER;
...
window.addEventListener(
  ...
  (e) => {
    if (state.isPinching && e.touches.length === 2) {
      ...
      const pinchCenterX = (a.clientX + b.clientX) / 2;
      const pinchCenterY = (a.clientY + b.clientY) / 2;
      zoomTo(state.pinchStartZoom * factor, pinchCenterX, pinchCenterY);
      return;
    }
    if (!state.isDragging) return;
    const touch = e.touches[0];
...
function zoomMap(factor, clientX, clientY) {
  zoomTo(state.zoom * factor, clientX, clientY);
}
...
function resetView() {
  ...
  const fitZoom = Math.min(visibleWidth / MAP_WIDTH, visibleHeight / MAP_HEIGHT);
  state.zoom = clampZoom(fitZoom * 0.98);

  const scaledWidth = MAP_WIDTH * state.zoom;
  const scaledHeight = MAP_HEIGHT * state.zoom;
  state.panX = visible.left + (visibleWidth - scaledWidth) / 2;
  state.panY = visible.top + (visibleHeight - scaledHeight) / 2;
  updateTransform();
}
...
function selectStore(name) {
  ...
  if (label) {
    ...
    // Fallback for shapes that don't use text-<normalized-name> ids.
    const shopGroup = unit.closest("g");
    if (shopGroup) {
      shopGroup
        .querySelectorAll(".text-unit")
...
function updateDetailLogo(store) {
  const badge = document.getElementById("storeLogoBadge");
  const img = document.getElementById("storeLogoBadgeImg");
  const fallback = document.getElementById("storeLogoBadgeFallback");
  if (!badge || !img || !fallback || !store) return;

  const name = translateStoreName(store.name, state.language) || store.name || "Store";
  fallback.textContent = (name[0] || "S").toUpperCase();

  if (store.logo && String(store.logo).trim()) {
    img.src = store.logo;
    img.alt = `${name} logo`;
    img.onerror = () => {
...
function initMapInteraction() {
  ...
  function showHoverFromStoreName(storeName, anchor = "center", pin = false) {
    ...
    state.hoverPinnedStoreName = pin ? storeName : "";
  ...
  window.addEventListener("map:transform", () => {
    if (!state.hoverPinned || !state.hoverPinnedStoreName) return;
    showHoverFromStoreName(state.hoverPinnedStoreName, "center", true);
  });
}
